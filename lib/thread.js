...
function createReporterConfigFile(path, settings) {
  const reporterEnabled = [
    '@landra_sistemas/cypress-parallel/json-stream.reporter.js'
  ];
  ...
  if (settings.reporter) {
    ...
    reporterEnabled.push(
      '@landra_sistemas/cypress-parallel/simple-spec.reporter.js'
    );
  }
  const content = {
...
function createCommandArguments(thread, settings) {
  ...
  if (settings.reporterOptionsPath) {
    ...
    createReporterConfigFile(reporterConfigPath, settings);
  }
...
async function executeThread(thread, index) {
  ...
  const commandArguments = createCommandArguments(thread, settings);
